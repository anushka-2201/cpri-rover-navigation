...
class UltrafastLaneDetector():
	...
	def process_output(output, cfg):		
		...
		lanes_points_arr = np.empty(len(lanes_points), dtype=object)
		for i, lp in enumerate(lanes_points):
			lanes_points_arr[i] = lp
		lanes_detected_arr = np.empty(len(lanes_detected), dtype=object)
		for i, ld in enumerate(lanes_detected):
			lanes_detected_arr[i] = ld
		return lanes_points_arr, lanes_detected_arr
